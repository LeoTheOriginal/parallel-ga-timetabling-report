...
\begin{enumerate}\small
    \item \textbf{Filtr strict 1.5h} --- akceptujemy tylko zdarzenia
          dokładnie pokrywające 7 standardowych bloków AGH
          (8:00--9:30, \ldots, 18:30--20:00) w~budynkach D-7/D-10/D-11.
          Odsiewamy egzaminy, kolokwia, NST, szkołę doktorską.
    \item \textbf{Deduplikacja} --- tygodniowe powtórzenia (14$\times$ wykład)
          do pojedynczej instancji. Klucz: (nazwa, grupa, typ, tytuł, dzień, prowadzący).
    \item \textbf{Mapowanie encji} --- niezależne ID dla sal (1--41),
          prowadzących (1--161), grup (1--191).
    \item \textbf{Grupy nadrzędne wykładów} --- prefix programu
          (np.~\texttt{FiIS-PFT-1}) staje się \emph{parent\_group} dla
...
\textbf{Wynik:} 4 znormalizowane CSV (\texttt{rooms}, \texttt{teachers},
\texttt{groups}, \texttt{courses}) + plan referencyjny UniTime.\\
Te same 4 pliki spożywa \texttt{io.c} bez zmian, dla wszystkich datasetów.
}}
\end{frame}
...
\vspace{0.5em}
\begin{itemize}\small
    \item \textbf{Deterministyczne} --- seed=42 gwarantuje powtarzalność
    \item \textbf{Stratyfikowane} --- każdy podzbiór zachowuje wszystkie
          encje (sale, prowadzący, grupy), nie tylko zdarzenia
    \item \textbf{Sprawiedliwe porównanie} --- czas $T_1$ vs $T_{16}$
          mierzymy dla \emph{tego samego} problemu, nie różnych
\end{itemize}

\vspace{0.6em}
\centering\small
Stąd 4 datasety w~tabeli wyników: \textbf{100, 200, 400, 1058} (UniTime full).
\end{frame}
...
Dla $n_e=100$: $f \approx 0.042 \Rightarrow S_\infty \approx 24\times$.
...
\begin{tabular}{rrrrl}
\toprule
\textbf{$n_e$} & \textbf{$S_{16}$} & \textbf{$S_\infty$} & \textbf{Reżim} \\
\midrule
100  & 11.96$\times$ & 24$\times$ & \cellcolor{warning}podliniowy --- mała praca/wyspę \\
200  & 24.55$\times$ & ---        & \cellcolor{ok}\textbf{super-liniowy} \\
400  & 28.73$\times$ & ---        & \cellcolor{ok}\textbf{super-liniowy} (optimum) \\
1058 & 19.59$\times$ & ---        & \cellcolor{warning}saturacja --- payload 8.5 KB \\
\bottomrule
\end{tabular}

\vspace{0.7em}
\textbf{Skąd $S_{16} > 16\times$?} Algorytm wyspowy ze migracją to nie
...
\begin{itemize}\small
    ...
    \item \textbf{Migracja jako wymiana wiedzy} --- co 50 pokoleń dobre cechy
          propagują się po pierścieniu, lokalne minima różnych wysp wybijają się wzajemnie
\end{itemize}

\vspace{0.4em}
\centering
\fcolorbox{black}{ok}{\parbox{0.85\textwidth}{\centering\scriptsize
Wariant 16-wyspowy daje też \textbf{lepsze rozwiązanie miękkie}:
soft$(T_1)=675 \to$ soft$(T_{16})=645$ ($-4.4\%$). Speedup nie jest jedynym efektem.
}}
\end{frame}
